	...
	function tagList(HyphaDomElement $pageListNode, $lang) {
		...
		if (!empty($selectedTagIds) || isUser()) {
			...
			if ((isUser() && !empty($deselectedTags)) || isAdmin()) {
				$html.= '<select class="remainingTags" onchange="selectTag(this.value);">';
...
<script>
	function selectTag(id) {
		if (!id) addTag();
			...
			hypha(<?=json_encode($url)?>, 'tagSelect', id, document.getElementById('<?=$formId?>'), function(response) {document.getElementById('tagList').innerHTML = response;});
			document.getElementById('tagList').innerHTML = 'loading...';
		}
	}
	function deselectTag(id) {
		hypha(<?=json_encode($url)?>, 'tagDeselect', id, document.getElementById('<?=$formId?>'), function(response) {document.getElementById('tagList').innerHTML = response;});
		document.getElementById('tagList').innerHTML = 'loading...';
	}
	function addTag() {
		var name = prompt(<?=json_encode(__('enter-label'))?>, '');
		if (name) {
			var description = prompt(<?=json_encode(__('enter-description'))?>, '');
			if (description) {
				var json = '{ "name" : "' + name + '", "description" : "' + description + '"}';
				hypha(<?=json_encode($url)?>, 'tagNew', json, document.getElementById('<?=$formId?>'), function(response) {document.getElementById('tagList').innerHTML = response;});
				document.getElementById('tagList').innerHTML = 'loading...';
			}
		}
	}
